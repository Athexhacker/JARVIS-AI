...
window.addEventListener("load", function () {
    // alert("Activate jarvis");
    setTimeout(() => {
        document.querySelector(".preloader").style.display = "none";
    }, 1000);
    speak('Activating jarvis');
    speak('going online');
    speak('hello mister hacker im wake up how can i help you');
    wishMe();
});
...
function speakThis(message) {
    ...
    if (message.includes('hi') || message.includes('hello') || message.includes('hey') || message.includes('hey')) {
        const finalText = greetings[Math.floor(Math.random() * greetings.length)];
        speech.text = finalText;
    }

    else if (message.includes('how are you') || message.includes('whats up')) {
        const finalText = 'I am fine athex tell me how can i help you';
        speech.text = finalText;
    }
    ...
    else if (message.includes('can you tell me about your self')) {
        const finalText = 'why not sir My name is jarvis and i creted by athex hacker';
        speech.text = finalText;
    }

    else if (message.includes('who i am')) {
        const finalText = 'your name is athex and youre dangerous and destroyer hacker';
        speech.text = finalText;

    }
    else if (message.includes('can you give me some information of your developer')){
        const finalText = 'ofcourse im jarvis created by inzi whos famous by athex name whos hacker and webdeveloper and ai expert if you need more information ill give her phone number'
    }
    else if (message.incude('can you tell me how i can contact to athex')){
        const finalText = 'why not his phone number is 03475549695'
    }

   else if (message.includes('activate jarvis')) {
    const finalText = 'G mister hacker jarvis is activated'
       speech.text = finalText;
   }
        ...
        window.open(`http://faceboom.com`, "_blank");
...
function wishMe() {
    ...
    var hr = day.getHours();
    if (hr >= 0 && hr < 12) {
        speak('Good Morning inzi');
    } 
    else if (hr == 12) {
        speak('Good Noon inzi');
    } 
    else if (hr >= 12 && hr <= 17) {
        speak('Good Afternoon inzi');
    } 
    else {
        speak('Good Evening inzi');
    }
}
